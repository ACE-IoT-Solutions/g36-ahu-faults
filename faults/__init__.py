...
class FaultConditionTwo:
    ...
    def apply(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["fc2_flag"] = (
            ...
            & (df[self.supply_vfd_speed_col] > .01)
        ).astype(int)
...
class FaultConditionTen:
    ...
    def apply(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['fc10_flag'] = (
            ...
            & (df[self.cooling_sig_col] > .01)
            & (df[self.economizer_sig_col] > .9)
        ).astype(int)
...
class FaultConditionEleven:
    ...
    def apply(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in [
            ...
            if df[col].max() > 1.0:
                raise TypeError(float_max_check_err(col))
